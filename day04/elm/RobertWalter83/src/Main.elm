...
init =
  let
    inputSorted = sortByTimestamp inputAsList
    map = mapIdToGuardInfoList inputSorted
  in
    { solution = (part1 map, part2 map) }


part1 : Dict Int GuardInfo -> Int
part1 map =
  let
    id = idOfGuardMostAsleepTotal map
    arrayWithHightime = Dict.get id map |> Maybe.withDefault Array.empty 
    (index, highTime) = findHightime (Array.toList arrayWithHightime) 0 (0, 0)
  in
    (id * index)
    
part2 : Dict Int GuardInfo -> Int
part2 map =
  let
    (id, index) = idOfGuardMostAsleepAtCertainMinute map
  in
    id * index
  

findHightime : List Int -> Int -> (Int, Int) -> (Int, Int)
findHightime list indexCurrent (index, hightime) =
  case list of
      [] ->
        (index, hightime)   
  
      head :: rest ->
        if head > hightime then
          findHightime rest (indexCurrent+1) (indexCurrent, head)
        else
          findHightime rest (indexCurrent+1) (index, hightime)
          

idOfGuardMostAsleepTotal : Dict Int GuardInfo -> Int
idOfGuardMostAsleepTotal map =
  let
      (key, sum) = Dict.foldl findMostAsleepTotal (0, 0) map
  in
    key

idOfGuardMostAsleepAtCertainMinute : Dict Int GuardInfo -> (Int, Int)
idOfGuardMostAsleepAtCertainMinute map =
  let
      (key, index, _) = Dict.foldl findMostAsleepCertainMinute (0, 0, 0) map
  in
    (key, index)

findMostAsleepTotal : Int -> GuardInfo -> (Int, Int) -> (Int, Int)
findMostAsleepTotal k v (key, sum) =
  let
    sumNew = Array.toList v |> List.sum  
  in
    if (sumNew > sum) then
      (k, sumNew)
    else
...
findMostAsleepCertainMinute : Int -> GuardInfo -> (Int, Int, Int) -> (Int, Int, Int)
findMostAsleepCertainMinute k v (key, index, hightime) =
  let
    (iNew, time) = Array.toIndexedList v |> 
                     List.foldl findMostAsleepCertainMinuteLocal (-1, -1) 
  in
    if time > hightime then
      (k, iNew, time)
    else
      (key, index, hightime)

findMostAsleepCertainMinuteLocal : (Int, Int) -> (Int, Int) -> (Int, Int)
findMostAsleepCertainMinuteLocal (iCandidate, tCandidate) (i, t) = 
  if tCandidate > t then (iCandidate, tCandidate) else (i, t)

mapIdToGuardInfoList : List String -> Dict Int GuardInfo
mapIdToGuardInfoList inputSorted =
  let
      (_, _, map) = List.foldl updateMapIdToGuardInfo (-1, 0, Dict.empty) inputSorted 
  in
    map

handleNewId : Int -> Dict Int GuardInfo -> Dict Int GuardInfo
handleNewId id map =
  if Dict.member id map then 
    map
  else
    Dict.insert id (Array.initialize 60 (\i -> 0)) map

handleAsleepAndAwake : String -> Int -> Int -> Dict Int GuardInfo -> (Int, Int, Dict Int GuardInfo)
handleAsleepAndAwake line idLast minLast map =
  let
    min = Maybe.withDefault 0 <| getMinuteFromLine line
  in
    if String.contains "asleep" line then
      (idLast, min, map)
    else
      if String.contains "wakes" line then
        ( idLast
        , min 
        , Dict.update idLast (updateMapEntryForGuard minLast (min-1)) map  
        )    
      else
        -- error case
        (idLast, minLast, map)

updateMinuteAsleep : Int -> GuardInfo -> GuardInfo
updateMinuteAsleep index guardInfo =
  let
    minCur = Array.get index guardInfo 
  in
    case minCur of
      Just min ->
        Array.set index (min+1) guardInfo
    
      Maybe.Nothing ->
        Array.set index 1 guardInfo

updateMapEntryForGuard : Int -> Int -> Maybe GuardInfo -> Maybe GuardInfo
updateMapEntryForGuard rangeStart rangeEnd mbGuardInfo =
  case mbGuardInfo of
    Just guardInfo ->
      Just 
        <| List.foldl updateMinuteAsleep guardInfo 
          <| List.range rangeStart rangeEnd
    Maybe.Nothing ->
      Maybe.Nothing

updateMapIdToGuardInfo : String -> (Int, Int, Dict Int GuardInfo) -> (Int, Int, Dict Int GuardInfo)
updateMapIdToGuardInfo line (idLast, minLast, map) =
  let
    mbId = getIdFromLine line
  in
    case mbId of
        Just id ->
          (id, 0, handleNewId id map)
    
        Maybe.Nothing ->
          handleAsleepAndAwake line idLast minLast map

sortByTimestamp : List String -> List String
sortByTimestamp listGuardInfo =
  listGuardInfo |>
    List.sortWith
      (\guardInfoLeft guardInfoRight -> 
        let
          timestampLeft = parseTimestamp guardInfoLeft
          timestampRight = parseTimestamp guardInfoRight

        in
          compareTimestamp timestampLeft timestampRight          
      ) 

compareTimestamp : List Int -> List Int -> Basics.Order
compareTimestamp tsLeft tsRight =
  case (tsLeft, tsRight) of
      ([], []) ->
          EQ
...
getIdFromLine : String -> Maybe Int
getIdFromLine line =
  extractIntBetween line ("#"," ")

getMinuteFromLine : String -> Maybe Int
getMinuteFromLine line =
  extractIntBetween line (":","]")

extractIntBetween : String -> (String, String) -> Maybe Int 
extractIntBetween line (leftSplitter, rightSplitter) = 
  String.split leftSplitter line |> 
    List.drop 1 |>
      List.head |>
        Maybe.withDefault "" |>
          String.split rightSplitter |>
            List.head |>
              Maybe.withDefault "" |>
                String.toInt

parseTimestamp : String -> List Int
parseTimestamp guardInfo =
  Regex.split splitterRegex guardInfo |>
