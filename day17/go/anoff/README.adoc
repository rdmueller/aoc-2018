...
For this I first created a map of all cavities that would end up as stationary water reservoirs once they get touched by a droplet of water. You can see the result of this here: link:../src/confined.txt. I iterate over all rows (starting bottom) once and for each tile I check if there are solid walls on both sides and that until the wall ``#`` is reached there is either solid clay ``#`` or another confined space ``o`` under it.
...
1. modify the current tile
  1. dirt ``.`` becomes moving water ``|``
  1. contained space ``o`` becomes stationary water ``~``
1. if the bottom tile is dirt ``.``
  1. _advance_ into bottom tile
1. if the bottom tile is a confined space ``o``
  1. _advance_ into bottom tile
  1. if left/right is dirt ``.`` and has a valid floor*, _advance_ to side
1. if left/right tile is confined space ``o``
  1. _advance_ to the side
1. if left/right tile is dirt ``.`` but the current tile has a valid floor* (this basically means it is the edge of something)
  1. _advance_ to the side
